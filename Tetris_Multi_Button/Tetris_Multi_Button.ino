...
#define KEY_DOWN 4
#define KEY_LEFT 5
#define KEY_ROT_90_DEG(k) ((k-KEY_UP)+3)%4+KEY_UP
#define KEY_ROT_180_DEG(k) ((k-KEY_UP)+2)%4+KEY_UP
#define KEY_ROT_270_DEG(k) ((k-KEY_UP)+1)%4+KEY_UP
...
byte keyLock = KEY_NONE;               // Holds the mode of the last keypress (for debounce and stuff)
...
ISR(PCINT0_vect) {
  if (keyLock == KEY_NONE) {
    keyLock = KEY_CONSUMED; // Nothing complicated to do here - this is picked up in the handleInput() function
  }
}
...
void playerIncTetris() {
  if (keyLock == KEY_NONE) {
    int ic = analogRead(1);
    if (ic > 800 && ic < 855) {
      keyLock = KEY_ROT_90_DEG(KEY_LEFT);
    } else if (ic > 855 && ic < 970) {
      keyLock = KEY_ROT_90_DEG(KEY_RIGHT);
    } else if (ic > 970 && ic < 1025) {
      keyLock = KEY_ROT_90_DEG(KEY_UP);
    } else if (ic > 630 && ic < 740) {
      keyLock = KEY_ROT_90_DEG(KEY_DOWN);
    }
    for (int i = 400; i < 800; i = i + 200) beep(20, i);
    //displayScore(ic, 1,80,0);
  }
}
...
void ssd1306_char_f8x8(uint8_t x, uint8_t y, const char ch[]) {
  ...
  while (ch[j] != '\0')
  {
    ...
    if (c > 15) c = c - 7;
    if (c>(64-12-7)) c = c - 32; //convert lower case to upper case
